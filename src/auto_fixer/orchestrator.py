...
class AutoTestFixerOrchestrator:
    ...
    def __init__(
        ...
        self.llm_fixer = LLMFixer(test_directory=test_directory)
        self.ast_patcher = ASTPatcher()

        # Track results
        self.fix_history: List[FixResult] = []
        self.code_bugs: List[TestFailure] = []
        self.app_env_errors: List[TestFailure] = []

        # Circuit breaker settings
        self.max_app_env_errors = 3  # Stop after seeing this many app env errors
        self.max_failures_per_iteration = 15  # Don't process more than this many failures per iteration
    ...
    def run(self, extra_pytest_args: List[str] = None) -> Dict[str, Any]:
        ...
        while iteration < self.max_iterations and not all_tests_fixed:
            ...
            if len(failures) > self.max_failures_per_iteration:
                print(f"⚠️ Limiting to {self.max_failures_per_iteration} failures (out of {len(failures)})")

            # Step 2-6: Process each failure
            test_mistakes_fixed = []
            code_bugs_found = []
            app_env_errors_found = []
            circuit_breaker_triggered = False

            for idx, failure in enumerate(failures_to_process, 1):
                print(f"\n Processing failure {idx}/{len(failures_to_process)}")
                print(f"Test: {failure.test_name} in {failure.test_file}")

                result = self._process_failure(failure)
                self.fix_history.append(result)

                if result.classification == "app_environment_error":
                    app_env_errors_found.append(failure)
                    self.app_env_errors.append(failure)
                    print(f"Classification: APP ENVIRONMENT ERROR (cannot fix - app needs external resources)")

                    # Circuit breaker: if we see too many app env errors, stop
                    if len(self.app_env_errors) >= self.max_app_env_errors:
                        print(f"\n🛑 CIRCUIT BREAKER: {len(self.app_env_errors)} app environment errors detected!")
                        print(f"The application code requires external resources (files, databases, services)")
                        print(f"that are not available in this environment.")
                        print(f"This CANNOT be fixed by modifying tests.")
                        circuit_breaker_triggered = True
                        break

                elif result.classification == "code_bug":
            ...
            print(f"App environment errors: {len(app_env_errors_found)}")
            if circuit_breaker_triggered:
                print(f"⚠️ Circuit breaker triggered - stopping auto-fixer")
            print(f"{'=' * 80}")
    ...
    def _process_failure(self, failure: TestFailure) -> FixResult:
        ...
        # Handle app environment errors immediately (cannot be fixed)
        if rule_classification == "app_environment_error":
            reason = self.rule_classifier.get_classification_reason(failure)
            print(f"Rule classifier: app_environment_error - {reason}")
            return FixResult(
                test_file=failure.test_file,
                test_name=failure.test_name,
                classification="app_environment_error",
                fix_attempted=False,
        ...
        # Handle app environment error from LLM (cannot be fixed)
        if llm_result.classification == "app_environment_error":
            return FixResult(
                test_file=failure.test_file,
                test_name=failure.test_name,
                classification="app_environment_error",
    ...
    def _apply_fix(self, failure: TestFailure, fixed_code: str) -> bool:
        ...
        # Resolve file path
        test_file_path = self._resolve_test_file_path(failure.test_file)
        ...
        if success:
            # Validate the patch
            if self.ast_patcher.validate_patch(test_file_path):
    ...
    def _resolve_test_file_path(self, test_file: str) -> str:
        ...
        Args:
            test_file: Test file path (may be relative)
        ...
        # Return as-is if not found (will error later with better message)
        return test_file
    ...
    def _read_test_function(self, failure: TestFailure) -> str:
        ...
        try:
            ...
            # If path is not absolute, try to resolve it relative to test_directory
            if not os.path.isabs(test_file_path):
                # Try relative to test_directory first
                candidate = os.path.join(self.test_directory, test_file_path)
                if os.path.exists(candidate):
                    test_file_path = candidate
                else:
                    # Try relative to project_root
                    candidate = os.path.join(self.project_root, test_file_path)
                    if os.path.exists(candidate):
                        test_file_path = candidate
            ...
            # For collection errors, return the ENTIRE file
            # (the error is at module level, not in a specific function)
            if failure.test_name == "__collection_error__":
                return content
            ...
            print(f"Error reading test function from {failure.test_file}: {e}")
            return ""
    ...
    def _generate_summary(self, iterations: int) -> Dict[str, Any]:
        ...
        app_env_errors = [r for r in unique_results if r.classification == "app_environment_error"]
        ...
        summary = {
            ...
            "app_environment_errors": len(app_env_errors),
            ...
            "app_env_errors_list": [f.to_dict() for f in self.app_env_errors]
        }
        ...
        if len(app_env_errors) > 0:
            print(f"⚠️ App environment errors (CANNOT fix): {len(app_env_errors)}")
            print(f"  These failures are due to the application code requiring")
            print(f"  external resources (files, databases, services) not available in CI.")
        print("=" * 80)
