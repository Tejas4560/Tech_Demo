...
ClassificationType = Literal["test_mistake", "code_bug", "app_environment_error"]
...
class LLMClassifier:
    ...
    - app_environment_error: Application code depends on missing external resources
    """
...
3. **app_environment_error**: The APPLICATION CODE (not the test) depends on external resources that don't exist in this environment. Examples:
   - App opens data files like `data/users.json` that don't exist
   - App connects to databases that aren't available
   - App requires environment variables that aren't set
   - App makes network calls to services that aren't running

IMPORTANT: If you see FileNotFoundError for data files, database connection errors, or missing environment variables in the APPLICATION CODE (not test code), classify as "app_environment_error". These CANNOT be fixed by modifying tests.
...
{
  "classification": "test_mistake" or "code_bug" or "app_environment_error",
...
{"classification": "app_environment_error", "reason": "App code opens data/questions.json which does not exist in CI", "fixed_code": null, "confidence": 0.95}
    ...
    def __init__(self, verbose: bool = False):
        ...
        if self.client is None:
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    "llm_client_llm_classifier",
                    str(llm_client_path)
                )
                llm_module = importlib.util.module_from_spec(spec)
                sys.modules['llm_client_llm_classifier'] = llm_module
                spec.loader.exec_module(llm_module)

                self.client = llm_module.get_openai_client()
                if verbose:
                    if llm_module.USE_GROQ:
                        print(f"🚀 Using Groq API for auto-fixer")
                    elif llm_module.USE_AZURE:
                        print(f"☁️  Using Azure OpenAI for auto-fixer")
            except Exception as e:
