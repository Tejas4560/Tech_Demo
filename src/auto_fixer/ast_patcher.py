...
class ASTPatcher:
    ...
    def patch_test_function_with_feedback(
        ...
        # SPECIAL CASE: Collection errors require replacing the ENTIRE file
        if test_function_name == "__collection_error__":
            print(f"  Fixing collection error - replacing entire file")
            patched_content = fixed_function_code

            # Validate the fixed code can be parsed
            try:
                ast.parse(patched_content)
            except SyntaxError as e:
                print(f"  Error: Fixed code has syntax error: {e}")
                return False, f"Syntax error in fixed code: {e}"
        else:
            # Parse original file
            try:
    ...
    def _test_fix_with_output(
        ...
        try:
            ...
            # SPECIAL CASE: For collection errors, just verify tests can be collected
            # We can't run a specific test because __collection_error__ isn't a real test name
            if test_function_name == "__collection_error__":
                print(f"  Validating collection error fix (checking if file can be collected)...")
                ...
                if result.returncode == 0:
                    print(f"  ✅ Fix validated - file can be collected!")
                    return True, ""
                else:
                    print(f"  ❌ Fix validation failed - file still has collection errors:")
            ...
            # Regular case: Run pytest on this specific test
