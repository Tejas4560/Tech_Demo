...
Given:
1. A failing test function (or entire test file for collection errors)
2. The error/traceback
3. Relevant source code being tested
4. Available pytest fixtures from conftest.py (if any)

Generate a fixed version of the test that will pass.
...
CRITICAL - COLLECTION ERRORS (errors that occur at module import time):
When you see errors like:
- "TypeError: HTTPException.__init__() missing 1 required positional argument: 'status_code'"
- "ImportError: cannot import name 'X' from 'Y'"
- "NameError: name 'X' is not defined"
- Any error happening at module level (not inside a test function)

For these COLLECTION ERRORS:
1. You'll receive the ENTIRE test file, not just a function
2. Look at the traceback to find the EXACT line causing the error
3. Fix that specific line - usually it's:
   * A raise statement missing arguments (HTTPException(status_code=401, detail="msg"))
   * A module-level import that fails
   * A decorator with wrong syntax
   * A global variable that's undefined
4. Return the ENTIRE FIXED FILE with all imports, decorators, and ALL functions
5. DO NOT remove or skip any test functions - keep them all
6. Make sure the file can be imported without any errors
