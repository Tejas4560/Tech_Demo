...
class FailureParser:
    ...
    def parse_failures(self, json_output: Dict[str, Any]) -> List[TestFailure]:
        ...
        # FIRST: Check for collection errors (import-time errors in test files)
        # These are NOT in "tests" but in "collectors" section
        collectors = json_output.get("collectors", [])
        for collector in collectors:
            ...
            if outcome == "failed":
                ...
                # Extract file and create a pseudo test name
                test_file = nodeid.split("::")[0] if "::" in nodeid else nodeid
                test_name = "__collection_error__"
                ...
                print(f"  Found collection error in: {test_file}")
