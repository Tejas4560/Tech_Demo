...
PYCODE
  ...
  if [ -f "$TARGET_DIR/requirements.txt" ]; then
    ...
    # Upgrade pip, setuptools, wheel for better compatibility with C-extensions
    pip install --upgrade pip setuptools wheel

    # Try normal install first, then fallback to binary-only if build fails
    if ! pip install -q -r "$TARGET_DIR/requirements.txt" 2>/dev/null; then
      echo "⚠️ Normal install failed, trying binary-only packages..."
      if ! pip install --only-binary=:all: -r "$TARGET_DIR/requirements.txt" 2>/dev/null; then
        echo "⚠️ Some dependencies could not be installed, trying to continue..."
        # Install what we can, ignore failures for problematic packages
        pip install -q -r "$TARGET_DIR/requirements.txt" --ignore-installed 2>/dev/null || true
      fi
    fi
...
PYCODE
  ...
  # Pre-flight check: Verify tests can be collected (detect import-time errors)
  echo "Checking test collection (detecting import-time errors)..."
  COLLECTION_OUTPUT=$(pytest "$CURRENT_DIR/tests/manual" --collect-only -q 2>&1) || true
  COLLECTION_EXIT_CODE=$?

  if echo "$COLLECTION_OUTPUT" | grep -q "FileNotFoundError\|No such file or directory"; then
    echo ""
    echo "🛑 CRITICAL: Test collection failed due to missing files!"
    echo "=================================================================="
    echo "The application code requires data files that don't exist in CI."
    echo ""
    echo "Error details:"
    echo "$COLLECTION_OUTPUT" | grep -A5 "FileNotFoundError\|No such file or directory" | head -20
    echo ""
    echo "=================================================================="
    echo "This is an APP ENVIRONMENT ERROR - cannot be fixed by modifying tests."
    echo "The target application needs data files like 'data/*.json' to run."
    echo ""
    echo "Solutions:"
    echo "  1. Add mock data files to the repository"
    echo "  2. Modify app code to handle missing files gracefully"
    echo "  3. Use environment-based configuration for data paths"
    echo ""

    # Generate summary with app_environment_error status
    echo "Generating pipeline summary..."
    python3 "$CURRENT_DIR/generate_summary.py" --workspace "$CURRENT_DIR" --output "$CURRENT_DIR/summary.json" 2>/dev/null || true

    exit 1
  fi

  if [ $COLLECTION_EXIT_CODE -ne 0 ]; then
    echo "⚠️ Warning: Test collection had issues (exit code: $COLLECTION_EXIT_CODE)"
    echo "Collection output:"
    echo "$COLLECTION_OUTPUT" | head -30
    echo ""
  fi
...
if [ "$TEST_COUNT" -gt 0 ]; then
  ...
  echo ""
  echo "Checking test collection (detecting import-time errors)..."
  COLLECTION_OUTPUT=$(pytest "$CURRENT_DIR/tests/generated" --collect-only -q 2>&1) || true
  COLLECTION_EXIT_CODE=$?
  ...
  if echo "$COLLECTION_OUTPUT" | grep -q "FileNotFoundError\|No such file or directory"; then
    ...
    # Generate summary with error status
    echo "Generating pipeline summary..."
    python3 "$CURRENT_DIR/generate_summary.py" --workspace "$CURRENT_DIR" --output "$CURRENT_DIR/summary.json" 2>/dev/null || true
